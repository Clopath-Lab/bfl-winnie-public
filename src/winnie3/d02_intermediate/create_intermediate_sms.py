...
def create_intermediate_received_sms(raw_received_sms):
    ...
    intermediate_received_sms.rename(
        columns={"message": "body", "date_sent": "message_sent_date"}, inplace=True
    )
    intermediate_received_sms["body"] = intermediate_received_sms["body"].apply(
...
def create_intermediate_sent_sms(raw_sent_sms):
    ...
    intermediate_sent_sms["body"] = intermediate_sent_sms["body"].apply(
        lambda x: clean_blasts(x)
    )
